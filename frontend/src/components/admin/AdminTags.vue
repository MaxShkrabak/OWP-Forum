...
const { loadRoles, roleLabel } = useAdminRoles();
...
async function loadTags() {
  ...
  try {
    ...
    error.value =
      e?.response?.data?.error || e.message || "Failed to load tags";
    tags.value = [];
  } finally {
    loading.value = false;
  }
...
function requestSave() {
  ...
  if (isDuplicateName(name, tags.value, "Name", "TagID", excludeId)) {
    return showInfo(
      "Duplicate tag",
      "That tag already exists. Please choose a different name.",
    );
  }
  ...
  const original = tags.value.find(
    (t) => Number(t.TagID) === Number(form.value.id),
  );
  ...
  showConfirm(
    ...
    async () => {
      closeModal();
      try {
        await updateTag(form.value.id, name, minRole);
        closeForm();
        await loadTags();
      } catch (e) {
        showInfo(
...
onMounted(async () => {
  await Promise.all([loadTags(), loadRoles()]);
});
</script>
            ...
            <tr v-for="t in filteredTags" :key="t.TagID">
              ...
              <td class="admin-minrole">
                ...
                <span class="role-short">{{
                  roleLabel(t.UsableByRoleID).charAt(0)
                }}</span>
              </td>
              <td>
                <div class="actions">
    ...
    >
      <div class="confirm-card">
        <h3 class="confirm-title">
          {{ form.mode === "add" ? "Add Tag" : "Edit Tag" }}
        </h3>
        <p class="confirm-subtitle">
          {{ form.mode === "add" ? "Create a new tag." : "Update the tag." }}
        </p>
