...
function updateActiveAttributes() {
  ...
  activeHighlightColor.value =
    editor.value.getAttributes("highlight").color || DEFAULT_HIGHLIGHT_COLOR;
  activeTextColor.value =
    editor.value.getAttributes("textStyle").color || DEFAULT_TEXT_COLOR;
}
...
const handleHeadingChange = (event) => {
  ...
  val === "paragraph"
    ...
    : chain.toggleHeading({ level: parseInt(val) }).run();
};
...
const handleClickOutside = (e) => {
  if (
    showLinkMenu.value &&
    linkGroupRef.value &&
    !linkGroupRef.value.contains(e.target)
  ) {
    showLinkMenu.value = false;
  }
};
  ...
  <div class="editor-container" :class="{ 'compact-mode': compact }">
    <div v-if="editor && showToolbar" class="tiptap-toolbar">
      ...
      <div class="color-group color-text">
        ...
        >
          <i
            class="bi bi-type fs-5"
            :style="{
              borderBottom: '4px ' + activeTextColor + ' solid',
              borderRadius: '4px',
            }"
          ></i>
        </button>
      ...
      <div class="color-group color-highlight">
        <button
          type="button"
          class="group-main-btn"
          @click="$refs.highlightInput.click()"
          title="Highlight Color"
        >
          <i
            class="bi bi-highlighter fs-5"
            :style="{
              borderBottom: '4px ' + activeHighlightColor + ' solid',
              borderRadius: '4px',
            }"
          ></i>
        </button>
        ...
        />
        <button
          v-if="isHighlightActive"
          type="button"
          class="reset-color"
          @click="resetHighlightColor"
        >
          <i class="bi bi-x"></i>
        </button>
      </div>
...
.tiptap-toolbar
  button:hover:not(.group-main-btn):not(.reset-color):not(.apply-btn),
.color-group:hover,
.link-group:hover,
